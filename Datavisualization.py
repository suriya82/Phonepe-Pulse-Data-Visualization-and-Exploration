...
conn = pymysql.connect(host='localhost', user='root', password='123456', db='phonepe')
cursor = conn.cursor()
...
if option == 'All India':
    ...
    with tab1:
        ...
        cursor.execute(f"SELECT State, Transaction_amount FROM agg_trans WHERE Year = '{in_tr_yr}' AND Quarter = '{in_tr_qtr}' AND Transaction_type = '{in_tr_tr_typ}';")
        ...
        cursor.execute(f"SELECT State, Transaction_count, Transaction_amount FROM agg_trans WHERE Year = '{in_tr_yr}' AND Quarter = '{in_tr_qtr}' AND Transaction_type = '{in_tr_tr_typ}';")
        ...
        cursor.execute(f"SELECT SUM(Transaction_amount), AVG(Transaction_amount) FROM agg_trans WHERE Year = '{in_tr_yr}' AND Quarter = '{in_tr_qtr}' AND Transaction_type = '{in_tr_tr_typ}';")
        ...
        cursor.execute(f"SELECT SUM(Transaction_count), AVG(Transaction_count) FROM agg_trans WHERE Year = '{in_tr_yr}' AND Quarter = '{in_tr_qtr}' AND Transaction_type = '{in_tr_tr_typ}';")
...
# ==============================================          /     State wise       /             ============================================== #
    ...
    with tab4:
        ...
        df_st_us_tab_qry_rslt1_fig = px.bar(df_st_us_tab_qry_rslt1 , x = 'Quarter', y ='Count', color ='Count', color_continuous_scale = 'thermal', title = 'User Analysis Chart', height = 500,)
